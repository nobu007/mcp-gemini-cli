...
describe('SpecificationService', () => {
  ...
  beforeEach(() => {
    ...
    specificationService = new SpecificationService(fileSystemService, nameGenerationService);
  });
  ...
  describe('initialize', () => {
    ...
    it('should call fileSystemService.getExistingSpecNames with correct path', async () => {
      ...
      expect(fileSystemService.getExistingSpecNames).toHaveBeenCalledWith('.kiro/specs');
    });
    ...
    it('should call nameGenerationService.generateUniqueName with description and existing names', async () => {
      ...
      fileSystemService.getExistingSpecNames = mock(() => Promise.resolve(existingNames));

      await specificationService.initialize('Test Feature');

      expect(nameGenerationService.generateUniqueName).toHaveBeenCalledWith('Test Feature', existingNames);
    });

    it('should create spec directory with correct path', async () => {
      await specificationService.initialize('My Feature');

      expect(fileSystemService.createDirectory).toHaveBeenCalledWith('.kiro/specs/new-feature');
    });
    ...
    it('should write spec.json file with correct structure', async () => {
      ...
      expect(specContent.approvals.requirements).toEqual({ generated: false, approved: false });
      expect(specContent.approvals.design).toEqual({ generated: false, approved: false });
      expect(specContent.approvals.tasks).toEqual({ generated: false, approved: false });
    });
    ...
    it('should reject empty description', async () => {
      ...
      if (result.isErr()) {
        expect(result.error.message).toBe('Project description cannot be empty');
      }
    });
    ...
    it('should return error when directory creation fails', async () => {
      fileSystemService.createDirectory = mock(() => Promise.resolve(err(new Error('Permission denied'))));

      const result = await specificationService.initialize('My Feature');

      expect(result.isErr()).toBe(true);
      if (result.isErr()) {
        expect(result.error.message).toContain('Failed to create spec directory');
        expect(result.error.message).toContain('Permission denied');
      }
    });

    it('should return error when file write fails', async () => {
      fileSystemService.writeFile = mock(() => Promise.resolve(err(new Error('Disk full'))));
    ...
    it('should use custom specs base path from config', async () => {
      ...
      expect(fileSystemService.getExistingSpecNames).toHaveBeenCalledWith('custom/path');
      expect(fileSystemService.createDirectory).toHaveBeenCalledWith('custom/path/new-feature');
    });
    ...
    it('should handle errors from getExistingSpecNames gracefully', async () => {
      fileSystemService.getExistingSpecNames = mock(() => Promise.reject(new Error('Read error')));
  ...
  describe('configuration', () => {
    it('should use default config when not provided', () => {
      const service = new SpecificationService(fileSystemService, nameGenerationService);

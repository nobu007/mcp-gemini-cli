...
describe("Schemas", () => {
  ...
  describe("TOOL_DEFINITIONS", () => {
    ...
    it("should have meaningful descriptions", () => {
      expect(TOOL_DEFINITIONS.googleSearch.description.length).toBeGreaterThan(
        10,
      );
      expect(TOOL_DEFINITIONS.geminiChat.description.length).toBeGreaterThan(
        10,
      );

      expect(TOOL_DEFINITIONS.googleSearch.description.toLowerCase()).toContain(
        "search",
      );
      expect(TOOL_DEFINITIONS.geminiChat.description.toLowerCase()).toContain(
        "chat",
      );
    });
  });

...
export class SpecificationService {
  ...
  async initialize(projectDescription: string): Promise<Result<{ featureName: string; specPath: string }, Error>> {
    try {
      // Validate input
      ...
      const existingNames = await this.fileSystemService.getExistingSpecNames(this.specsBasePath);

      // Generate unique feature name
      const featureName = this.nameGenerationService.generateUniqueName(projectDescription, existingNames);
      ...
      if (dirResult.isErr()) {
        return err(new Error(`Failed to create spec directory: ${dirResult.error.message}`));
      }
      ...
      if (writeResult.isErr()) {
        return err(new Error(`Failed to write spec file: ${writeResult.error.message}`));
      }

...
var ElicitResultSchema = ResultSchema.extend({
  action: exports_external.enum(["accept", "reject", "cancel"]),
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (code === 0) {
        ...
        reject(new Error("gemini not found globally and --allow-npx option not specified."));
      }
    });
    child.on("error", (err) => {
      reject(err);
...
async function executeGeminiCli(geminiCliCommand, args, timeoutMs = Number.parseInt(process.env.GEMINI_CLI_TIMEOUT_MS || "60000")) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      if (!isResolved) {
        isResolved = true;
        child.kill("SIGTERM");
        reject(new Error(`gemini-cli operation timed out after ${timeoutMs}ms`));
      }
    }, timeoutMs);
    child.stdin.end();
...
var GoogleSearchParametersSchema = exports_external.object({
  query: exports_external.string().describe("The search query."),
  limit: exports_external.number().optional().describe("Maximum number of results to return (optional)."),
  raw: exports_external.boolean().optional().describe("Return raw search results with URLs and snippets (optional)."),
...
async function executeGeminiChat(args, allowNpx = false) {
...
async function main() {
  ...
  try {
    ...
    console.error("Please install gemini-cli globally or use --allow-npx option.");
    process.exit(1);
  }
  ...
  server.registerTool("googleSearch", {
    description: "Performs a Google search using gemini-cli and returns structured results.",
  ...
  server.registerTool("geminiChat", {
...
//# debugId=1008C8A8CFE10E6B64756E2164756E21
